...
class App(tk.Tk):
    def __init__(self):
        ...
        self.geometry = self.config_data.get('Window', 'geometry')
        self.title('Coders Notebook')
        ...
        self.mode = 'file'
        self.font_family = self.config_data.get('Font', 'family')
        self.font_size = self.config_data.get('Font', 'size')
        self.fontColor = self.config_data.get('Colors', 'foreground')
        self.fontBackground = self.config_data.get('Colors', 'background')
...
#        self.nb.show_lines(e=None)
        ...
        self.filetree.treetoggle_button = tk.Button(self.filetree, text="Folder ⮟", anchor="w", bd=0, relief='flat', command=self.tree_toggle, width=31)
...
#            self.bind("<Key>", self.nb.show_lines)
    ...
    def right_click_tab(self, event):
        self.menubar.tab_right_click_menu.post(event.x_root, event.y_root)
    ...
    def open_folder(self):
        ...
        # load the treeview with directory contents
        self.make_tv(folder)
        # Display contents of treeview
        self.filetree.tv.pack(expand='true', anchor="n", fill='both')
        # Refresh frame
        self.filetree.pack_forget()
        self.filetree.pack(side='left', fill='both')
...
#        self.nb.linenums.delete()        
    ...
    def make_tv(self, folder):
        ...
        self.filetree.tv.delete(*self.filetree.tv.get_children())
        self.tv_items = {}
        self.file_contents = {}
...
#            self.nb.show_lines(e=None)
            ...
            self.nb.current_tab().textbox.pack_forget()
            self.editor_status = "hidden"
            messagebox.showerror('Error', "Couldn't display file.")
            self.nb.current_tab().textbox.delete("1.0","end")
            self.nb.current_tab().textbox.insert("1.0", "There was an error displaying the file.")
            self.nb.current_tab().textbox.configure(state='disabled')

    # --------------------- formatting functions -------------------#

    def set_tags(self):
        page = self.nb.current_tab().textbox
        self.current_tags = page.tag_names()
        ...
        if "overstrike" in self.current_tags:
            overstrike = 1
        else:
            overstrike = 0
        self.big_font = font.Font(page, page.cget("font"))
        self.big_font.configure(slant = slant , weight = weight , underline = underline , overstrike = overstrike , family = self.font_family , size = self.font_size)
        page.tag_config("BigTag", font = self.big_font , foreground = self.fontColor , background = self.fontBackground) 
        if "BigTag" in  self.current_tags:
            page.tag_remove("BigTag" , 1.0 , tk.END)
            page.tag_add("BigTag" , 1.0 , tk.END)
    ...
    def bold(self, event=None):
        ...
        if page.tag_ranges(tk.SEL):
            bold_font = font.Font(page, page.cget("font"))
            bold_font.configure(weight="bold")
            page.tag_configure("bold", font=bold_font)
            self.current_tags = page.tag_names()
            if "bold" in self.current_tags:
    	    	# first char is bold, so unbold the range
                page.tag_delete("bold", "sel.first", "sel.last")
            else:
    ...
    def slant(self):
        ...
        if page.tag_ranges(tk.SEL):
            self.current_tags = page.tag_names()
            if "italic" in self.current_tags:
                page.tag_remove("italic", "sel.first", "sel.last")
            else:
                page.tag_add("italic", "sel.first", "sel.last")
    ...
    def show_tree(self):
        self.filetree.treetoggle_button.pack(side='top', fill='both')
        self.filetree.treetoggle_button.configure(text="Folder ⮟", width=31, anchor='w', font=('JetBrains Mono', 8))
        self.filetree.tv.pack(expand=True, anchor='n', fill='both')
    ...
    def check(self, value):
        self.nb.current_tab().textbox.tag_remove('found', '1.0', tk.END)
        self.nb.current_tab().textbox.tag_config('found', foreground='red')
        ...
        for word in list_of_words:
            idx = '1.0'
            while idx:
                idx = self.nb.current_tab().textbox.search(word, idx, nocase=1, stopindex=tk.END)
                if idx:
                    wordlist[idx] = word
                    lastidx = '%s+%dc' % (idx, len(word))
                    self.nb.current_tab().textbox.tag_add('found', idx, lastidx)
                    idx = lastidx
                    count +=1
        self.result_label.config(text="There were "+ str(count) + " occurences")            
        print(wordlist)

    def find_text(self, event=None):
        self.nb.search_toplevel = tk.Toplevel(self.nb)
        self.nb.search_toplevel.geometry("350x80")
        self.nb.search_toplevel.title('Find Text')
        self.nb.search_toplevel.transient(self.nb)
        self.nb.search_toplevel.resizable(False, False)
        self.findframe=tk.Frame(self.nb.search_toplevel)
        tk.Label(self.findframe, text="Find:").grid(row=0, column=0, sticky='e')
        self.search_entry_widget = tk.Entry(self.findframe, width=25)
        self.search_entry_widget.grid(row=0, column=1, padx=2, pady=2, sticky='we')
        self.search_entry_widget.focus_set()
        self.findframe.grid(row=0, column=0)
        # Create a label to show the number of occurrences
        self.result_label = tk.Label(self.nb.search_toplevel, text="")
        self.result_label.grid(row=1, column=0, padx=3, pady=3, sticky='we')
        tk.Button(self.nb.search_toplevel, text="Ok", underline=0, command=lambda: self.check(self.search_entry_widget.get())).grid(row=0, column=2, sticky='e' +'w', padx=2, pady=5)
        tk.Button(self.nb.search_toplevel, text="Cancel", underline=0, command=lambda: self.find_text_cancel_button(self.nb.search_toplevel)).grid(row=0, column=4, columnspan=2, sticky='e' +'w', padx=2, pady=2)

    def find_text_cancel_button(self, item):
        self.nb.current_tab().textbox.tag_remove('found', '1.0', tk.END)
        item.destroy()
        return "break"
...
class Tab(ttk.Frame):
    ...
    def create_text_widget(self):
        ...
        textbox = tk.Text(self, relief='sunken',
                                borderwidth=0,
                                wrap='none',
                                font=(self.config_data.get('Font', 'family'), self.config_data.get('Font', 'size')))
...
class Menuclass(tk.Menu):
    def __init__(self, parent):
        ...
        self.format_sub_menu = tk.Menu(self.file_menu)
        ...
        self.tab_right_click_menu.add_command(label="Close Tab", command=self.parent.close_tab)
    ...
    def edit_menu_item(self):
        ...
        self.edit_menu.add_cascade(menu=self.format_sub_menu, label='Format text')
        self.format_sub_menu.add_command(label = 'Bold', accelerator='Ctrl+b', command=self.parent.bold)
        self.format_sub_menu.add_command(label = 'Italics', accelerator='Ctrl+i', command=self.parent.slant)
        self.edit_menu.add_separator()
        self.edit_menu.add_command(label = 'Find', accelerator='Ctrl+f', command=self.parent.find_text)
    ...
    def database_menu_item(self):
        ...
        # Oracle Menu Items
        self.oracle_sub_menu.add_command(label = 'Connections')
        ...
        # Postgres Menu Items
        self.postgres_sub_menu.add_command(label = 'Connections')
        self.postgres_sub_menu.add_command(label = 'Databases')
        self.postgres_sub_menu.add_command(label = 'Objects')
        self.postgres_sub_menu.add_command(label = 'Filesystems')
